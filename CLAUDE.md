...
- **Charts** (`drawBarChart`, `monotonePath`, `niceTicks`) are hand-drawn SVG, no charting library. The progress line chart (`drawLineChart` → `paintLineChart`) is the interactive one: pinch/wheel zoom, drag to pan, tap a dot to open the entry behind it, and a fullscreen copy (`renderChartFull`). Its view (`ui.chartView`) and selection (`ui.chartSel`) live in `ui` so a re-render never resets them, in-flight pointer state is module-level (`chartPointers`/`chartGesture`) because a gesture redraws the chart underneath itself, and **every `[data-linechart]` on the page is painted from the same state** — the card and the fullscreen window are one chart drawn twice. Selecting a point deliberately patches `[data-linedetail]` in place rather than calling `render()`.
- **The 1RM calculator tab** (`renderCalc`) is stand-alone: nothing it does is logged. Its maths is `calcPct`/`calcReps` at the top of `app.js` — Epley read backwards, in two pieces that meet at 10 reps / 75% — and its tables are built from the *unrounded* max so they round exactly once.
